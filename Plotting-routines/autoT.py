...
outfile = 'ms2stream-analysis' #Name of outfile
...
#selection =[1,2] #Selection of simulations to compare *** ALL=[-1] ***
...
home = '/scratch_lg/renyu-group/mscheucher/EPACRIS-1/Result/'  #home directory
path = home  #
...
def USstd76():
    ...
    Hb = [old_div((r0*Hb[i]),(r0-Hb[i])) for i in range(0,len(Hb))] #H->Z
    ...
    for i in range(0,len(Hstd)):
        ...
        if (Hstd[i]>=91.0 and Hstd[i]<=110.0):
            Tstd[i] = Tc + AA * math.sqrt(1 - ((old_div((Hstd[i]-91),a))**2))
