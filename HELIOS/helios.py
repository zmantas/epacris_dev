...
def run_helios():
    ...
    if os.path.exists('keeper.pkl'):
        print ("Found keeper, skipping opacity read!")
        with open('keeper.pkl', 'rb') as f:
            keeper = pickle.load(f)        
        keeper_on = True
    else:
        keeper = quant.Store()
    ...
    if keeper.opacity_mixing == "premixed":
        ...
        if keeper_on == False:
            ...
            # Save the 'keeper' object to a file
            with open('keeper.pkl', 'wb') as f:
                pickle.dump(keeper, f)
    ...
    if writeall:
        ...
        print("Only saving TP and flux files")
        writer.write_tp(keeper, reader)
        writer.write_integrated_flux(keeper, reader)
...
def main():
    ...
    if __name__ == "__main__":
        ...
        run_helios()  # Pass the 'keeper' object to the 'run_helios' function
main()
