...
# =============================================================================
                ...
                # Instead of aborting at max steps, jump to convection
                if quant.iter_value > quant.max_nr_iterations:
                    quant.T_lay = quant.dev_T_lay.get()
                    condition2 = quant.T_lay[quant.nlayer] < quant.plancktable_dim * quant.plancktable_step - 2
                    quant.convection = 1
...
# =============================================================================
                if quant.iter_value > quant.max_nr_iterations:  # warning: hardcoded number
                    print("\nRun exceeds allowed maximum allowed number of iteration steps. Breaking the loop...")
                    write.write_abort_file(quant, read)
                    condition = False 
    ...
    def calculate_total_opacity_and_scat_cross_sections_from_species(self, quant):
        ...
        # Print the names of each species in the species list

        # Print the abundance of H2O if it exists in the species list
        if quant.iter_value == 0:
            h2o_found = False
            for species in quant.species_list:
                if species.name == "H2O":
                    h2o_found = True
                    print("\nH2O abundance profile:")
                    print("Layer   Pressure (dyn/cm²)   VMR")
                    print("-----   -----------------   -------")
                    for i in range(len(species.vmr_layer)):
                        print("{:5d}   {:.4e}           {:.4e}".format(
                            i, quant.p_lay[i], species.vmr_layer[i]))
                    print("\nH2O abundance summary:")
                    print("  - Layer VMR min: {:.4e}".format(np.min(species.vmr_layer)))
                    print("  - Layer VMR max: {:.4e}".format(np.max(species.vmr_layer)))
                    print("  - Layer VMR mean: {:.4e}".format(np.mean(species.vmr_layer)))
                    break
