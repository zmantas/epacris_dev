...
def initial_temp(quant, read):
    ...
    if quant.singlewalk == 0 and (quant.force_start_tp_from_file == 0 or quant.physical_tstep == 0):
        ...
        quant.T_lay = np.ones(quant.nlayer+1) * max(T_eff, 150)

        print("\nStarting with an isothermal TP-profile at {:g}".format(max(T_eff, 150))+" K.")
...
def mark_convective_layers(quant, stitching):
    ...
    if stitching == 1:
        if quant.iter_value > 1000:  # warning: hardcoded number
