...
import argparse
try:
    from . import phys_const as pc
except ImportError:
    import phys_const as pc
try:
    from . import planet_database as pd
except ImportError:
    import planet_database as pd
try:
    from . import species_database as sdb
except ImportError:
    import species_database as sdb
try:
    from . import host_functions as hsfunc
except ImportError:
...
class Read(object):
            ...
            for line in pfile:
                ...
                if column:
                    ...
                    # iteration
                        ...
                        print (self.temp_format)
                        print (self.temp_pressure_unit)
    ...
    def read_species_file(self, quant):
        ...
        # # Forces the chemistry to be read from vmr file during coupling even if FastChem is set in the species file
        # if quant.coupling == 1 and self.force_eq_chem == "no":
        #     print('Forcing VMR chemistry!')
        #     for s in range(len(quant.species_list)):

        #         if quant.species_list[s].source_for_vmr == "FastChem":
        #             quant.species_list[s].source_for_vmr = "file"
    ...
    def read_vertical_vmr_and_interpolate_to_helios_press_grid(vmr_file, species, file_press, helios_press):
        ...
        if ("CIA" not in species.name) and ("H-" not in species.name) and (species.name != "He-"):
            ...
            #print (vmr_file.dtype.names)
            vertical_vmr = vmr_file["H_m"]
