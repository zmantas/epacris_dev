 ...
 * Version: v0x, 2023: Now with Tvar being layer center temperatures
 *******************************************************************
...
//    printf("%s %d\n", "ms_d2stream jTvar =",j);
        Tdoub[0] = Tvar[0][j]; //TOA boundary (not adjusted)
        for (i=1;i<=zbin;i++) //initialize temperature vector: cell centers 
        {
            Tdoub[2*i-1] = Tvar[i][j];
        }
        Tdoub[2*zbin] = Tvar[zbin][j]; //Surface (also varied!)
        for (i=2;i<2*zbin;i=i+2) //initialize temperature vector: cell interfaces
        {
...
//=== Calculate Planck functions and derivatives ==========
        for(i=0; i<=2*zbin; i++) //Layloop 2; Thermal atmospheric radiation
        {
            ...
            B[i] /= exp(HPLANCK*CLIGHT / wavelength[lbin]/1e-9/KBOLTZMANN/Tdoub[i]) - 1.0 ;
            if(i>0)
            {
                if(TAUdoub[i] > 1.0e-4) dB[i] = (B[i] - B[i-1]) / TAUdoub[i] / ( 2*E[i]*(1 - w[i]*g[i])  ); //assumes opacities const. over Tvar range... 
                else 
                {
                    dB[i] = 0.0; //isothermal approach where no absorption
                    //B[i] = 0.5*(B[i] + B[i-1]); //ms2023: see Deitrick+2020
                    //B[i-1] = 0.5*(B[i] + B[i-1]); //ms2023: see Deitrick+2020
                }
            }
        }//END: Layloop 2
