...
void RadConv(double tempeq[], double P[], double T[], double Tint, char outnewtemp[])
{
    ...
    if (IFRC==0) {
        ...
        /* determine convection, adjust, and record convective layer */
        for (j=zbin; j>=1; j--) {
            if ( tempb[j-1] >= (tempb[j] * pow(P[j-1]/P[j],lapse[j])) ) {
    ...
    for (i=1; i<=NMAX_RC; i++) {
        ...
        for (rcount=1; rcount<=1; rcount++) { /* roughlt balance radiation between each convective update */
            ...
            if (  (Rfluxmax < 1.0 * SIGMA*pow(Tint,4.0))  ) {
                for (j=0; j<=zbin; j++) {
