...
void RadFlux(double Rflux[], double tempbnew[], double P[], int ncl, int isconv[], double lapse[], double **xxf, double T[], double Tint, int stdn[], double qysum[], double **cross, double **crosst, double romega);
...
void RadFlux(double Rflux[], double tempbnew[], double P[], int ncl, int isconv[], double lapse[], double **xxf, double T[], double Tint, int stdn[], double qysum[], double **cross, double **crosst, double romega)
{
    ...
    int isconv1[zbin+1],lapse1[zbin+1],radcount;
    for (k=1; k<=zbin; k++) {
	...
	double surfaceref, surfaceemi;
	double taumaxi;
	
	double solarfraction = 0.5;
    ...
    FILE *fp, *fopa;
    fp=fopen("checkemission.dat","w");
    fopa=fopen("checkopacity.dat","w");
	...
	for (i=0; i<(NWave-1); i++) {
		...
		for (j=1; j <= zbin; j++) {
			...
			/* for (k=1; k<=nump; k++) {
				...
				wa[j] += xxf[j1][stdn[k]]*crossl/qysum[k];
			}
			wa[j] += opacCO2[j1][i]*xxf[j1][52]/MM[j1];
			wa[j] += opacO2[j1][i]*xxf[j1][54]/MM[j1];*/
            wa[j] += opacH2O[j1][i]*xxf[j1][7]/MM[j1];
            wa[j] += opacCH4[j1][i]*xxf[j1][21]/MM[j1];
			wa[j] += opacNH3[j1][i]*xxf[j1][9]/MM[j1];
            
			/*wa[j] += opacSO2[j1][i]*xxf[j1][43];
			wa[j] += opacOH[j1][i]*xxf[j1][4];
			wa[j] += opacH2CO[j1][i]*xxf[j1][22];
			wa[j] += opacH2O2[j1][i]*xxf[j1][6];
			wa[j] += opacHO2[j1][i]*xxf[j1][5];
			wa[j] += opacH2S[j1][i]*xxf[j1][45];
			wa[j] += opacCO[j1][i]*xxf[j1][20];
			wa[j] += opacO3[j1][i]*xxf[j1][2];
			wa[j] += opacC2H2[j1][i]*xxf[j1][27];
			wa[j] += opacC2H4[j1][i]*xxf[j1][29];
			wa[j] += opacC2H6[j1][i]*xxf[j1][31];
			wa[j] += opacHCN[j1][i]*xxf[j1][37];
			wa[j] += opacCH2O2[j1][i]*xxf[j1][23];
			wa[j] += opacHNO3[j1][i]*xxf[j1][18];
			wa[j] += opacN2O[j1][i]*xxf[j1][11];
			wa[j] += opacN2[j1][i]*xxf[j1][55];
			wa[j] += opacNO[j1][i]*xxf[j1][12];
			wa[j] += opacNO2[j1][i]*xxf[j1][13];
			wa[j] += opacOCS[j1][i]*xxf[j1][49]; */
			
            wa[j] += H2CIA[j1][i]*xxf[j1][53]*xxf[j1][53]/MM[j1];
            wa[j] += H2HeCIA[j1][i]*xxf[j1][53]*heliumnumber[j1]/MM[j1];
			
            
            /* wa[j] += N2CIA[j1][i]*xxf[j1][55]*xxf[j1][55]; */
			/* wa[j] += CO2CIA[j1][i]*xxf[j1][52]*xxf[j1][52];*/
			/* wa[j] += crossa[1][i]*xxf[j1][78]*98.0/mole2dust*(1.0-sinab[1][i]);
			wa[j] += crossa[2][i]*xxf[j1][111]*256.0/mole2dust*(1.0-sinab[2][i]);
			ws[j] += crossa[1][i]*xxf[j1][78]*98.0/mole2dust*sinab[1][i];
			ws[j] += crossa[2][i]*xxf[j1][111]*256.0/mole2dust*sinab[2][i]; */
		...
		for (j=1; j <= zbin; j++) {
			g[j] = 0.0;
			j1   = zbin+1-j;
			/*g[j] += crossa[1][i]*xxf[j1][78]*98.0/mole2dust*sinab[1][i]*asym[1][i];
			g[j] += crossa[2][i]*xxf[j1][111]*256.0/mole2dust*sinab[2][i]*asym[2][i];*/
		...
		for (j=1; j <= zbin; j++) {
            j1   = zbin+1-j;
			tau[j] = (wa[j] + ws[j])*NAVOGADRO/meanmolecular[j1]/GA*(P[j1-1]-P[j1])*0.1;
			/* check */
		...
		if (wavelength[i]>3000.0) {
			taumaxi = TAUMAX2;
			surfaceemi = PSURFEM;
			surfaceref = 1.0-surfaceemi;
		} else {
			taumaxi = TAUMAX1;
			surfaceref = PSURFAB;
			surfaceemi = 1.0-surfaceref;
		}
        ...
        for (kk=0; kk<=nrl+1; kk++) {
			...
			E[l] += surfaceemi*PI*2*HPLANCK*pow(CLIGHT,2)/pow(lam1[i],5)/(exp(HPLANCK*CLIGHT/lam1[i]/KBOLTZMANN/temp1[ntau])-1.0)*1.0E-9;
            ...
            radiationI1 += cos(THETAREF)*solar[i]*solarfraction*exp(-tc[zbin]/cos(THETAREF))*(wavelength[i+1]-wavelength[i]); /* bottom of atmosphere */
            radiationI0 += cos(THETAREF)*solar[i]*solarfraction*(wavelength[i+1]-wavelength[i]); /* top of atmosphere */
            radiationO += dflux[0]*(wavelength[i+1]-wavelength[i]);
    ...
    printf("%s\t%f\t%f\t%f\n", "radiationI", radiationI0, radiationI1, radiationO);
