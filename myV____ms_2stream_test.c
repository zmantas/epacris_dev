...
// DIRECT_BEAM_MODE is now defined in the planet configuration file (e.g., TOI-199b.h)
...
//=== POPULATE helper terms (mostly Heng+2018) ============
    ...
    //angular parameters - HELIOS-consistent implementation
        // HELIOS-style switch: use either direct beam OR diffuse, never both
        #if DIRECT_BEAM_MODE == 1
            // Pure direct beam mode (like HELIOS dir_beam = 1)
            muD[i] = cos(THETAREF);
            ffrac[i] = 1.0;  // No diffuse factor
            //if (i == 1) printf("EPACRIS: Direct beam mode - muD=%.3f, ffrac=%.3f, THETAREF=%.1f deg\n", muD[i], ffrac[i], THETAREF*180.0/PI);
        #else
            // Pure diffuse mode (like HELIOS dir_beam = 0)
            muD[i] = 1.0;    // No zenith angle effect
            ffrac[i] = FADV; // Global energy distribution
            //if (i == 1) printf("EPACRIS: Diffuse mode - muD=%.3f, ffrac=%.3f, FADV=%.3f\n", muD[i], ffrac[i], FADV);
        #endif
    ...
    //2-stream closures
       ...
       // eps2[i] = 0.5; // deep atmosphere - 100% absorption approximation
        ...
        // HELIOS-consistent direct beam terms
        #if DIRECT_BEAM_MODE == 1
            // Direct beam mode: include direct beam terms (like HELIOS dir_beam = 1)
        ...
        #else
            // Diffuse mode: zero direct beam terms (like HELIOS dir_beam = 0)
            gammaD[i] = 0.0;  // No direct beam component
            CD[i] = 0.0;      // No direct beam coefficients
            CP[i] = 0.0;
            CM[i] = 0.0;
        #endif
...
//printf("%s\n", "Filling RT flux matrix"); //template
        //TOA:
            ...
            Bmat[0] = 0.0;//penta
            //RHS: Source term
            // HELIOS-consistent TOA boundary condition
            #if DIRECT_BEAM_MODE == 1
                // Direct beam mode: use zenith angle (like HELIOS dir_beam = 1)
                Smat[0] = muD[1]*solar[lbin]; // Direct beam flux only
            #else
                // Diffuse mode: use global energy factor (like HELIOS dir_beam = 0)
                Smat[0] = ffrac[1]*solar[lbin]; // Diffuse flux only
            #endif
        //BOA:
            //LHS: Flux terms
            Emat[4*zbin+1] = 0.0;
