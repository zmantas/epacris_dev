...
//ms2023		}
        ...
        printf("Top    T: %-10.2f  P: %-10.2e\n", T[zbin], P[zbin]);
        printf("Bottom T: %-10.2f  P: %-10.2e\n", T[0], P[0]);
        }
...
//**************************************************************
    /* Iteration */
    double TVARTOTAL = 0.0;
    double chemistry_diff = 1.0;
    int outer_iter = 0;
    const double CHEM_TOL = 1e-4;  // Tolerance for chemistry convergence

    // Outer loop for chemistry-radiation coupling
    while ((TVARTOTAL > Tol_RC_T || chemistry_diff > CHEM_TOL) && outer_iter < NMAX) {
        outer_iter++;
        
        // Store previous chemistry state for convergence check
        double prev_chemistry[zbin+1][NSP+1];
        for (j=1; j<=zbin; j++) {
            for (i=1; i<=NSP; i++) {
                prev_chemistry[j][i] = xx[j][i];
            }
        }

        // 1. Run radiative-convective calculation to convergence
        if(RadConv_Solver == 1) {
        ...
        // 2. Calculate chemical equilibrium with condensation
        if (IMODE == 0) {
            // Store current condensate state
            double clouds_backup[zbin+1][NSP+1];
            for (j=1; j<=zbin; j++) {
                for (i=1; i<=NSP; i++) {
                    clouds_backup[j][i] = clouds[j][i];
                }
            }
            ...
            // Update abundances while preserving condensates
            for (j=1; j<=zbin; j++) {
                for (i=1; i<=numx; i++) {
                    Con[(j-1)*numx+i] = MM[j]*mixequil[j][i];
                }
        ...
        // 3. Update mean molecular mass and opacities
        for (j=1; j<=zbin; j++) {
            totalnumber = 0.0;
            totalmass = 0.0;
            for (i=1; i<=numx; i++) {
        ...
        // Calculate chemistry convergence
        chemistry_diff = 0.0;
        for (j=1; j<=zbin; j++) {
            for (i=1; i<=NSP; i++) {
                chemistry_diff = fmax(chemistry_diff, fabs(xx[j][i] - prev_chemistry[j][i])/fmax(xx[j][i], 1e-30));
            }
        }
        ...
        printf("Outer iteration %d: T variation = %g K, Chemistry variation = %g\n", 
               outer_iter, TVARTOTAL, chemistry_diff);
    }

    // Final chemistry calculation to ensure consistency
    if (IMODE == 0) {
        mixequil=dmatrix(1,zbin,1,numx+numf);
        chemquil(pl, tl, zbin+1, labels, numx+numf, mixequil, atomfile);
        checkmixequil(numx+numf, mixequil);
