...
{	
	...
	// Print wavelength range information for debugging
	printf("Wavelength range for interpolation: %.2f to %.2f nm\n", 
		   wavelength[0], wavelength[NLAMBDA-1]);

	// Print sample values from the first loaded CIA file
	if (h2h2_cache.is_loaded && h2h2_cache.cia_data != NULL) {
		printf("First 3 wavelengths in H2H2 file: %.2f, %.2f, %.2f nm\n",
			h2h2_cache.wavelength[0], h2h2_cache.wavelength[1], h2h2_cache.wavelength[2]);
		printf("Sample values at first wavelength: %.2e, %.2e, %.2e\n",
			h2h2_cache.cia_data[0][0], h2h2_cache.cia_data[0][1], h2h2_cache.cia_data[0][2]);
	}
}
