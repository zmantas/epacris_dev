...
void RadFlux(double Rflux[], double tempbnew[], double P[], int ncl, int isconv[], double lapse[], double T[], double Tint)
{
    ...
    /* Determine number of convective layers, record the bottom radiative layer */
    int isconv1[zbin+1],radcount,convcount,nconv;
    int *id_br, *sign_br, *sign_br_old, sign_br_sum;
    ...
    convcount = 0;
    for (k=1; k<zbin; k++) {
        if (isconv1[k]==0 && isconv1[k+1]==1) {
            convcount = convcount + 1;
        }
    }
    nconv = convcount;
    id_br = ivector(1,nconv); /* in 1-nrl, not 1-zbin */
    sign_br = ivector(1,nconv);
    sign_br_old = ivector(1,nconv);
    ...
    double tempvarc;
    Tvar = dmatrix(0,zbin,0,nrl+1+nconv);
    for (j=0; j<=zbin; j++) {
        Tvar[j][0] = T[zbin-j];
    }
    for (j=0; j<=zbin; j++) {
    ...
    for (k=1; k<=zbin; k++) {
        if (isconv1[k]==0) { /* radiative layer, perturb temperature */
            ...
            /* pertub temperature in convective layer as additional cases */
            if (k<zbin) {
                if (isconv1[k]==0 && isconv1[k+1]==1) {
                    convcount = convcount+1;
                    id_br[convcount] = radcount;
                    sign_br[convcount] = 1;
                    for (j=0; j<=zbin; j++) {
                        Tvar[j][nrl+1+convcount] = T[zbin-j];
                    }
                    tempvarc=tempvar;
                    Tvar[k][nrl+1+convcount] = Tvar[k][nrl+1+convcount]+tempvarc;
                    kk=1;
                    while (k+kk<=zbin && isconv1[k+kk] == 1) {
                        tempvarc  = tempvarc*pow(P[zbin-k-kk]/P[zbin-k-kk+1],lapse1[k+kk]);
                        Tvar[k+kk][nrl+1+convcount] = Tvar[k+kk][nrl+1+convcount]+tempvarc;
                        kk = kk+1;
                    }
                }
            }
    ...
    /* Net flux */
    double **NetFlux;
    NetFlux = dmatrix(0,nrl,0,nrl+1+nconv);
    
	for (j=0; j<=nrl; j++) {
        for (k=0; k<=nrl+1+nconv; k++) {
            NetFlux[j][k] = 0.0;
        }
	}
    ...
    while (sign_br_sum != 0) {
        ...
        for (j=0; j<=nrl; j++) {
            ...
            for (kk=1; kk<=nconv; kk++) {
                if (sign_br[kk]==0) {
                    jmax[j+1][id_br[kk]+1] = (NetFlux[j][nrl+1+kk] - NetFlux[j][0])/tempvar;
                }
            }
        }
        /* Solve Equation */
        ...
        /* Sign of bottom rad layer */
        sign_br_sum = 0;
        for (j=1; j<=nconv; j++) {
            sign_br_old[j] = sign_br[j];
            if (deltaT[id_br[j]+1]<0) {
                sign_br[j] = 0;
            } else {
                sign_br[j] = 1;
            }
            sign_br_sum += abs(sign_br_old[j]-sign_br[j]);
        }
        /* print test */
        radcount = 0;
        for (j=1; j<=zbin; j++) {
            if (isconv1[j]==0) {
                radcount = radcount+1;
                printf("%e %d %f\n", P[zbin-j], isconv1[j], deltaT[radcount+1]);
            } else {
                printf("%e %d\n", P[zbin-j], isconv1[j]);
            }
        }
    ...
    free_dmatrix(NetFlux,0,nrl,0,nrl+1+nconv);
    
    
    /* calculate the new temperature profile */
    for (j=1; j<=nrl+1; j++) {
        deltaT[j] *= R_RELAX; /* relaxation */
    }
        ...
        if (isconv1[k]==0) { /* radiative layer, perturb temperature */
            ...
            if (k<zbin) {
                if (isconv1[k]==0 && isconv1[k+1]==1) {
                    ...
                    }/* update temperature in the convective layer when bottom radiation layer temperture decrease */
                }
            }
        }
    }
    ...
    free_dmatrix(Tvar,0,zbin,0,nrl+1+nconv);
    free_dvector(deltaT,1,nrl+1);
    free_ivector(indx,1,nrl+1);
    free_ivector(id_br,1,nconv);
    free_ivector(sign_br,1,nconv);
    free_ivector(sign_br_old,1,nconv);
