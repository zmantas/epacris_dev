...
//=========================================================
//=== Helper Functions ====================================
// Note: The condensed heat capacity functions are now in condensed_heat.c
// and included via condensed_heat.h header
...
//    if(lay==1) printf("%s%e\n","lapse = ",lapse[lay]);
    ...
    // GRAHAM+2021 APPROACH: Handle rainout with proper mass conservation
    // Calculate total mole fraction removed by rainout
    double total_rainout_fraction = 0.0;
    for (i=0; i<NCONDENSIBLES; i++) {
        total_rainout_fraction += (1.0 - alpha[i]) * Xc[i];
    }
    
    // Calculate remaining atmospheric fraction
    double remaining_fraction = 1.0 - total_rainout_fraction;
    
    if (total_rainout_fraction > 1.0e-15 && remaining_fraction > 1.0e-15) {
        // GRAHAM METHOD: Renormalize ALL atmospheric abundances
        // This maintains the total atmospheric pressure/density
        
        // First update the condensed species with renormalization
        for (i=0; i<NCONDENSIBLES; i++) {
            // Gas phase: equilibrium vapor, renormalized by remaining fraction
            xx[lay][CONDENSIBLES[i]] = (Xv[i] / remaining_fraction) * MM[lay];
            
            // Retained cloud fraction, renormalized
            clouds[lay][CONDENSIBLES[i]] = (alpha[i] * Xc[i] / remaining_fraction) * MM[lay];
            
            // Debug output for rainout
            if(lay==120 && (Xc[i] > 1.0e-12 || i==0)) {
                double rainout_mass = (1.0 - alpha[i]) * Xc[i] * MM[lay];
                printf("DEBUG: Layer %d, Species %d - Final state: gas = %.6e, cloud = %.6e, rained_out = %.6e\n",
                       lay, CONDENSIBLES[i], xx[lay][CONDENSIBLES[i]], clouds[lay][CONDENSIBLES[i]], rainout_mass);
            }
        }
        
        // Then renormalize ALL other species proportionally
        for (j=1; j<=NSP; j++) {
            // Check if this species is already handled as a condensible
            int is_condensible = 0;
            for (i=0; i<NCONDENSIBLES; i++) {
                if (j == CONDENSIBLES[i]) {
                    is_condensible = 1;
                    break;
            ...
            // Only renormalize non-condensible species
            if (!is_condensible) {
                xx[lay][j] /= remaining_fraction;
            }
        }
        
        
        // // Debug output for first layer
        // if(lay==1) {
        //     for (i=0; i<NCONDENSIBLES; i++) {
        //         if (Xc[i] > 1.0e-12) {
        //             printf("  Species %d: Xv_norm = %.6e, Xc_retained_norm = %.6e, rained_out_fraction = %.6e\n",
        //                    CONDENSIBLES[i], xx[lay][CONDENSIBLES[i]]/MM[lay], 
        //                    clouds[lay][CONDENSIBLES[i]]/MM[lay], (1.0 - alpha[i]) * Xc[i]);
        //         }
        //     }
        // }
    } else {
        // No significant rainout, use original values without renormalization
        for (i=0; i<NCONDENSIBLES; i++) {
            xx[lay][CONDENSIBLES[i]] = Xv[i] * MM[lay]; 
            clouds[lay][CONDENSIBLES[i]] = alpha[i] * Xc[i] * MM[lay];
            
            // Debug output for no rainout case
            // if(lay==1 && (Xc[i] > 1.0e-12 || i==0)) {
            //     printf("DEBUG: Layer %d, Species %d (no rainout) - Final state: gas = %.6e, cloud = %.6e\n",
